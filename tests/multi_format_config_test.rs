...
fn test_detect_format_from_path() {
    ...
    assert_eq!(
        ConfigFormat::detect_from_path("./config.txt"),
        None
    );
    assert_eq!(
        ConfigFormat::detect_from_path("./config"),
        None
    );
}
...
fn test_convert_with_explicit_format() {
    ...
    let result = ConfigLoader::convert_config_with_format(
        input_file,
        output_file,
        ConfigFormat::Yaml
    );
    assert!(result.is_ok(), "Should convert with explicit format successfully");
    ...
    assert!(output_content.contains("Authorizations:"), "Output should contain YAML format");
...
fn test_convert_preserves_data() {
    ...
    let json_content = r#"{
      "Authorizations": [
        {
          "Name": "TestAuth",
          "AuthorizationInfo": "Bearer token123"
        }
      ],
      "ServiceItems": [
        {
    ...
    assert_eq!(
        original_config.service_items[0].api_name,
        converted_config.service_items[0].api_name,
...
fn test_convert_json_to_yaml() {
    ...
    assert!(fs::metadata(output_file).is_ok(), "Output YAML file should exist");
...
"#;
    ...
    assert!(fs::metadata(output_file).is_ok(), "Output TOML file should exist");
...
"#;
    ...
    assert!(fs::metadata(output_file).is_ok(), "Output JSON file should exist");
...
fn test_load_json_config() {
    ...
    assert!(!config.service_items.is_empty(), "Should have service items");
}
