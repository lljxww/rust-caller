...
async fn test_update_with_combined_params() {
    ...
    // 验证更新后的数据
    assert_eq!(api_result.str_at("title"), Some("Updated Title"));
    assert_eq!(api_result.str_at("body"), Some("Updated body content"));
...
async fn test_patch_with_combined_params() {
    ...
    // 验证部分更新的数据
    assert_eq!(api_result.str_at("title"), Some("Patched Title"));
    assert_eq!(api_result.i64_at("id"), Some(1));
}
...
async fn test_combined_params_missing_path() {
    // 测试：组合参数类型缺少路径参数应该失败
    let params = HashMap::from([("title".to_string(), "Test".to_string())]);
    ...
    assert!(
        result.is_err(),
        "Should fail when path parameters are missing"
    );

    match result.unwrap_err() {
        caller::CallerError::MissingPathParameter { name: param } => {
            assert_eq!(
                param, "post_id",
                "Error should mention missing post_id parameter"
            );
        }
        _ => panic!("Expected MissingPathParameter for missing path parameters"),
    }
}
