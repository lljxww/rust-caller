...
fn add_service(builder: &mut ConfigBuilder) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth = read_line("Authorization type (optional): ")?;
...
fn edit_service(builder: &mut ConfigBuilder) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let new_url = read_line(&format!("New base URL (enter to keep '{}'): ", service.base_url))?;
    ...
    let new_auth = read_line(&format!("New auth type (current: {}, enter to keep): ", current_auth))?;
    
    let current_timeout = service.timeout.unwrap_or(30000);
    let new_timeout = read_line(&format!("New timeout (current: {}ms, enter to keep): ", current_timeout))?;
