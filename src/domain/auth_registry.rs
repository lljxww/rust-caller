...
use super::auth_trait::{AuthProvider, AuthContext, Authenticator};
...
/// # Example
/// ```rust
/// use caller::domain::auth_registry::AuthRegistry;
/// use caller::domain::builtin_auth::BearerAuth;
/// 
...
impl AuthRegistry {
    ...
    pub fn register(name: &str, auth: impl Authenticator + 'static) -> Result<(), CallerError> {
        ...
        let mut providers = AuthRegistry::global()
            .providers
            .write()
            .map_err(|e| CallerError::AuthenticationError(format!("Failed to acquire write lock: {}", e)))?;
    ...
    pub fn list() -> Result<Vec<String>, CallerError> {
        let providers = AuthRegistry::global()
            .providers
            .read()
            .map_err(|e| CallerError::AuthenticationError(format!("Failed to acquire read lock: {}", e)))?;
