...
use caller::{CallParams, RetryConfig, call_params, call_params_with_retry, params};
...
async fn main() -> Result<(), caller::CallerError> {
    ...
    println!(
        "   JSON: {}",
        serde_json::to_string_pretty(&params.to_json()?).unwrap()
    );
    println!();
